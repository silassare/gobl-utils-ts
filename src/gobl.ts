...
export const getEntityCache = (entity_name: string) => gobl_cache[entity_name];

export const c_bool = (v: any): boolean => {
	// @ts-ignore
	return v === null || v === undefined ? v : Boolean(v === '0' ? 0 : v);
};

export const c_int = (v: any): number => {
	// @ts-ignore
	return v === null || v === undefined ? v : parseInt(v);
};

export const c_string = (v: any): string => {
	// @ts-ignore
